  ...
  <div style="display:flex;flex:1">
    ...
    <div style="width:400px;flex-shrink:0;padding:32px 24px 32px 32px;display:flex;align-items:flex-start">
      <div style="width:100%;border-radius:2px;overflow:hidden;border:1px solid #e5e0d8;box-shadow:0 2px 12px #f7f5f2">
        <img src="${imgUrl}" alt="Garment" style="width:100%;height:auto;display:block" crossorigin="anonymous" />
      </div>
    </div>
      ...
      <table style="width:100%;border-collapse:collapse;color:#2d2422">
        <thead>
          <tr style="border-bottom:2px solid #2d2422">
            <th style="padding:12px 16px 12px 0;text-align:left;font-size:16px;font-weight:900;letter-spacing:0.1em;text-transform:uppercase;opacity:0.6;width:220px">测量部位 Measure Point</th>
      ...
      <div className="fixed inset-0 z-[200] flex flex-col bg-[#2d2422] overflow-hidden">
        ...
        >
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[13px] font-semibold text-[#2d2422] transition-all"
            style={{ background: 'rgba(139,115,85,0.06)', border: '1px solid rgba(139,115,85,0.1)' }}
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-bold text-white transition-all active:scale-95"
        ...
        {/* 移动端滚动提示 */}
        <div className="md:hidden text-center py-2 text-[11px] text-[#9b8e82] flex-shrink-0">
          ← 左右滑动查看完整预览 →
        </div>
      </div>
    )
            ...
            <div className="lg:col-span-4 space-y-5">
              ...
              <div className="fashion-glass rounded-2xl p-4 md:p-5">
                <div className="mb-3">
                  ...
                  <p className="text-[12px] text-[#9b8e82] mt-1">上传欧美女装图片，AI 自动解析款式与 S 码尺寸</p>
                </div>
                <ImageUploader
                  label="样照"
                  value={imageUrl}
              ...
              <div className="fashion-glass rounded-2xl p-4 md:p-5">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-semibold text-[#b0a59a] tracking-wide uppercase mb-1.5 block">款式名称</label>
                    <input
                      className="w-full px-3 py-2.5 rounded-xl text-[13px] font-semibold text-[#2d2422] outline-none transition-all"
                      style={{ background: 'rgba(139,115,85,0.03)', border: '1px solid rgba(139,115,85,0.1)' }}
                      value={productData.name}
                      onChange={(e) => setProductData({ ...productData, name: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-semibold text-[#b0a59a] tracking-wide uppercase mb-1.5 block">主面料材质</label>
                    <input
                      className="w-full px-3 py-2.5 rounded-xl text-[13px] font-semibold text-[#2d2422] outline-none transition-all"
                      style={{ background: 'rgba(139,115,85,0.03)', border: '1px solid rgba(139,115,85,0.1)' }}
                  ...
                  <div>
                    <label className="text-[11px] font-semibold text-[#b0a59a] tracking-wide uppercase mb-1.5 block">辅料配件描述</label>
                    <textarea
                      className="w-full px-3 py-2.5 rounded-xl text-[13px] font-semibold text-[#2d2422] outline-none transition-all resize-none"
                      style={{ background: 'rgba(139,115,85,0.03)', border: '1px solid rgba(139,115,85,0.1)' }}
                      rows={2}
              ...
              <div className="fashion-glass rounded-2xl p-4 md:p-5">
                <button
                  ...
                  className="w-full flex items-center justify-between text-[14px] font-bold text-[#2d2422]"
                >
                  <span className="flex items-center gap-2">
                    <History size={16} className="text-[#c67b5c]" /> 历史记录
                    {history.length > 0 && (
                      <span className="text-[10px] font-bold text-[#9b8e82] bg-[rgba(139,115,85,0.06)] px-1.5 py-0.5 rounded">{history.length}</span>
                    )}
                  </span>
                  <ChevronLeft size={16} className={`text-[#b0a59a] transition-transform ${showHistory ? '-rotate-90' : 'rotate-0'}`} />
                </button>

                {showHistory && (
                  <div className="mt-3 space-y-2 max-h-[360px] overflow-y-auto">
                    {history.length === 0 ? (
                      <p className="text-[12px] text-[#b0a59a] text-center py-6">暂无历史记录</p>
                    ) : (
                      history.map(record => (
                        <div
                          key={record.id}
                          className="group flex items-start gap-2.5 p-2.5 rounded-xl transition-all cursor-pointer hover:bg-[rgba(139,115,85,0.04)]"
                          style={{ border: '1px solid rgba(139,115,85,0.06)' }}
                        ...
                        >
                          ...
                          />
                          <div className="flex-1 min-w-0">
                            <div className="text-[12px] font-bold text-[#2d2422] truncate">{record.productData.name}</div>
                            <div className="text-[10px] text-[#b0a59a] flex items-center gap-1 mt-0.5">
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-1 rounded-lg hover:bg-[rgba(196,112,112,0.08)] transition-all flex-shrink-0"
                          >
            ...
            <div className="lg:col-span-8">
              {isProcessing ? (
                <div className="h-full flex flex-col items-center justify-center py-20 fashion-glass rounded-2xl">
                  ...
                  <p className="font-bold text-[#2d2422] text-lg mb-1">AI 正在解析欧美女装样照...</p>
                  <p className="text-[12px] text-[#9b8e82]">提取表格尺寸数据并按规则进行全码数推导</p>
                </div>
              ) : showTable ? (
                <div className="space-y-5">
                  ...
                  <div className="flex justify-between items-center px-1">
                    <div>
                      <h2 className="text-lg font-bold text-[#2d2422]">AI 自动生成全码规格单</h2>
                      <p className="text-[11px] text-[#9b8e82] mt-0.5">Base S: 支持编辑，修改后全码联动</p>
                    </div>
                    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold text-white"
                      ...
                      <table className="w-full text-sm min-w-[480px] md:min-w-0 md:table-fixed">
                        <thead>
                          <tr className="bg-[#2d2422] text-white">
                            ...
                            {productData.specs.map(s => (
                              <th key={s.size} className={`p-2 md:p-4 text-center font-bold text-sm md:text-base ${s.size === 'S' ? 'bg-[#c67b5c]' : ''}`}>
                                {s.size}
                        ...
                        <tbody className="divide-y divide-[rgba(139,115,85,0.06)]">
                          {([
                            ...
                            <tr key={row.k} className="hover:bg-[rgba(139,115,85,0.02)] transition-colors">
                              <td className="p-2 md:p-4 font-bold text-[#2d2422] whitespace-nowrap">
                                {row.l} <span className="text-[10px] text-[#b0a59a] ml-1 font-normal uppercase hidden lg:inline">{row.e}</span>
                              </td>
                              {productData.specs.map(s => (
                                <td key={s.size} className={`p-1.5 md:p-2 text-center text-xs md:text-sm min-w-[56px] md:min-w-0 ${s.size === 'S' ? 'bg-[rgba(198,123,92,0.06)]' : 'font-bold text-[#6b5d4f]'}`}>
                                  {s.size === 'S' ? (
                                    <input
                                      type="number"
                                      step="0.5"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <button
                      ...
                      className="py-3.5 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 text-[#8b7355] transition-all active:scale-[0.98]"
                      style={{ background: 'rgba(139,115,85,0.04)', border: '2px solid rgba(139,115,85,0.12)' }}
                    >
                  ...
                  >
                    <div className="p-2.5 rounded-xl shadow-sm flex-shrink-0" style={{ background: 'rgba(198,123,92,0.08)' }}>
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#8b7355] leading-relaxed font-medium">
                ...
                <div className="h-full flex flex-col items-center justify-center py-28 fashion-glass rounded-2xl">
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4"
                    style={{ background: 'rgba(139,115,85,0.06)' }}
                  >
                    <ImageIcon size={32} className="text-[#b0a59a]" />
                  </div>
                  <p className="text-[#6b5d4f] font-bold text-center px-6">请在左侧上传欧美女装图片解析</p>
                  <p className="text-[10px] text-[#b0a59a] mt-2 px-10 text-center uppercase tracking-widest font-semibold">
              ...
              {/* Error */}
              {error && (
                <div className="mt-4 flex items-center gap-2 text-[#c47070] text-xs p-3 rounded-xl font-medium"
