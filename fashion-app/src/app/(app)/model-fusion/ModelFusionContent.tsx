...
export default function ModelFusionContent() {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-[var(--bg-page)] overflow-hidden">
      ...
      >
        ...
        <div className="w-8 h-8 rounded-2xl hidden md:flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #ec4899, #f43f5e)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_440px] gap-6">
        ...
        <div className="flex flex-col gap-6 min-w-0">
          {/* 顶部 Tab 切换 */}
          <div className="flex gap-2 p-1 rounded-2xl" style={{ background: 'var(--bg-muted)', border: '1px solid var(--border-light)' }}>
            {([
      ...
      {tab === 'fusion' && (
          ...
          {modelCount > 1 && (
            <div className="fashion-glass rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-[var(--text-primary)]">融合策略</h3>
              </div>
                ...
                {STRATEGIES.map(s => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-3 px-4 py-3 rounded-2xl text-left transition-all border"
                      style={{
        ...
        <aside className="lg:sticky lg:top-4 self-start w-full">
          <div className="fashion-glass rounded-2xl p-3 md:p-5 min-h-[320px] md:min-h-[520px] flex flex-col">
            ...
            <div className="flex-1 rounded-2xl border border-dashed border-[var(--border-strong)] bg-[var(--bg-muted)] flex items-center justify-center overflow-hidden p-3 md:p-4 min-h-[240px] md:min-h-[420px]">
              {submitting ? (
                ...
                <div className="w-full flex flex-wrap justify-center gap-3">
                  {resultUrls.map((url, i) => (
                    <img
                      ...
                      className="max-w-full max-h-[420px] object-contain rounded-2xl shadow-[0_4px_18px_rgba(0,0,0,0.1)] cursor-pointer hover:shadow-[0_8px_28px_rgba(0,0,0,0.16)] transition-all"
                    />
                  ))}
                </div>
              )}
...
function SelectField({ label, value, options, onChange, hint }: {
  ...
  const dropdown = open && pos && (
    <>
      ...
      <div
        className="fixed z-[201] max-h-[240px] overflow-y-auto rounded-2xl bg-[var(--bg-card)] border border-[var(--border-normal)] shadow-lg py-1"
    ...
    <div className="flex flex-col gap-1 min-w-0">
      ...
      <div className="md:hidden relative">
        <button
          ...
          className="w-full text-left rounded-[10px] border bg-[var(--bg-muted)] border-[var(--border-normal)] outline-none transition-all"
          style={{ padding: '8px 28px 8px 10px', fontSize: '13px', color: 'var(--text-primary)' }}
        >
          <span className="block truncate">{displayLabel}</span>
          <svg className="absolute right-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--text-quaternary)] pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
