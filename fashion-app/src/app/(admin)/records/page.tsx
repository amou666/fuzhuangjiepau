            ...
            <tbody>
              {records.map((record) => {
                const typeMap: Record<string, { label: string; icon: React.ReactNode; cls: string }> = {
                  'workspace': { label: '工作台', icon: <Sparkles className="w-3 h-3" />, cls: 'bg-[rgba(198,123,92,0.08)] text-[#c67b5c]' },
                  'model-fusion': { label: '模特合成', icon: <Drama className="w-3 h-3" />, cls: 'bg-[rgba(139,115,85,0.08)] text-[#8b7355]' },
                  ...
                  <td className="px-4 py-3 border-b border-gray-100">
                    <div className="flex gap-1">
                      {refImages.slice(0, 3).map((img, i) => (
                        <img key={i} src={img.url} alt={img.label} onClick={() => setPreviewImage(img.url)} className="w-10 h-10 object-cover rounded-lg cursor-pointer border border-gray-200 hover:ring-2 hover:ring-blue-300 transition-all" title={img.label} />
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3 border-b border-gray-100">
                  ...
                  <td className="px-4 py-3 border-b border-gray-100">
                    <div className="flex gap-1.5 items-center flex-wrap">
                      {resultImages.slice(0, 4).map((url, i) => (
                        <img key={i} src={url} alt={`结果${i+1}`} onClick={() => setPreviewImage(url)} className="w-10 h-10 object-cover rounded-lg cursor-pointer border border-gray-200 hover:ring-2 hover:ring-blue-300 transition-all" />
                      ))}
                      {resultImages.length > 4 && <span className="text-[10px] text-gray-400">+{resultImages.length - 4}</span>}
                      {resultImages.length > 0 && (
                        <button className="inline-flex items-center gap-0.5 px-2 py-1 bg-white/70 text-gray-700 border border-black/10 rounded-lg text-[10px] font-medium hover:bg-white/90 transition-all" onClick={() => handleDownload(resultImages[0], record.id)}>
                          <Download className="w-2.5 h-2.5" /> 下载
                        </button>
                      )}
                      {record.upscaledUrl && (
                        <img src={record.upscaledUrl} alt={`放大${record.upscaleFactor}x`} onClick={() => setPreviewImage(record.upscaledUrl!)} className="w-10 h-10 object-cover rounded-lg cursor-pointer border-2 border-green-400" title={`已放大${record.upscaleFactor}x`} />
                      )}
      ...
      {upscaleModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[1000] p-6" onClick={() => !upscaleLoading && setUpscaleModal(null)}>
          <div className="relative max-w-[480px] w-full bg-white/85 backdrop-blur-[40px] rounded-3xl border border-white/80 shadow-2xl p-8" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              className="w-full mt-4 inline-flex items-center justify-center px-5 py-2.5 bg-white/70 text-gray-700 border border-black/10 rounded-xl text-sm font-medium backdrop-blur-sm hover:bg-white/90 transition-all disabled:opacity-50"
