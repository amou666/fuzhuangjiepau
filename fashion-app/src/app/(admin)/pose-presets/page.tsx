...
export default function PosePresetsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="fashion-glass rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                className={`w-full px-3 py-2 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 resize-none ${generatingPrompt ? 'border-blue-300 bg-blue-50/30' : 'border-gray-200'}`}
                rows={2}
                value={form.prompt}
                onChange={(e) => {
              ...
              <div className="flex items-center gap-3">
                {form.thumbnailUrl ? (
                  <div className="relative">
                    <img src={form.thumbnailUrl} alt="" className="w-12 h-16 rounded-lg object-cover border border-gray-200" />
                    <button
                      type="button"
                ...
                <button
                  type="button"
                  className="inline-flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-xl text-xs text-gray-500 hover:bg-gray-50 transition-colors"
      ...
      <div className="flex gap-2">
        <button
          type="button"
          className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${filterCategory === 'all' ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              key={cat.value}
              type="button"
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${filterCategory === cat.value ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                ...
                {filteredPresets.map((p) => (
                  <tr key={p.id} className="hover:bg-blue-500/[0.03] transition-colors">
                      ...
                      {editingId === p.id ? (
                        <div className="flex items-center gap-2">
                          {editForm.thumbnailUrl ? (
                            <img src={editForm.thumbnailUrl} alt="" className="w-9 h-12 rounded-lg object-cover border border-gray-200" />
                          ) : (
                            <div className="w-9 h-12 rounded-lg bg-gray-100 flex items-center justify-center">
                              <ImageIcon className="w-3 h-3 text-gray-300" />
                            </div>
                          )}
                          <button
                            type="button"
                            className="inline-flex items-center gap-1 px-2 py-1 border border-gray-200 rounded-lg text-[10px] text-gray-400 hover:bg-gray-50"
                        ...
                        <img src={p.thumbnailUrl} alt="" className="w-9 h-12 rounded-lg object-cover border border-gray-200" />
                      ) : (
                        <div className="w-9 h-12 rounded-lg bg-gray-100 flex items-center justify-center">
                          <ImageIcon className="w-3 h-3 text-gray-300" />
                        </div>
                    ...
                    <td className="px-4 py-3 border-b border-gray-100">
                      {editingId === p.id ? (
                        <input
                          className="px-2 py-1 border border-gray-200 rounded-lg text-sm w-28 focus:outline-none focus:ring-2 focus:ring-blue-200"
                    ...
                    <td className="px-4 py-3 border-b border-gray-100">
                      {editingId === p.id ? (
                        <select
                          className="px-2 py-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
                    ...
                    <td className="px-4 py-3 border-b border-gray-100 max-w-[260px]">
                      {editingId === p.id ? (
                        <div className="relative">
                          ...
                          <textarea
                            className={`w-full px-2 py-1 border rounded-lg text-xs resize-none focus:outline-none focus:ring-2 focus:ring-blue-200 ${editGeneratingPrompt ? 'border-blue-300 bg-blue-50/30' : 'border-gray-200'}`}
                            rows={2}
                    ...
                    <td className="px-4 py-3 border-b border-gray-100 text-gray-400 text-xs">
                      {editingId === p.id ? (
                        <input
                          type="number"
                          className="w-16 px-2 py-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
                      ...
                      <div className="flex items-center gap-1">
                        {editingId === p.id ? (
                          <>
                            <button
                              type="button"
                              className="inline-flex items-center gap-1 px-2 py-1 text-green-600 hover:bg-green-50 rounded-lg text-xs transition-all"
                            ...
                            <button
                              type="button"
                              className="inline-flex items-center gap-1 px-2 py-1 text-gray-400 hover:bg-gray-50 rounded-lg text-xs transition-all"
                          ...
                          <>
                            <button
                              type="button"
                              className="inline-flex items-center gap-1 px-2 py-1 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg text-xs transition-all"
                              onClick={() => startEdit(p)}
                            >
