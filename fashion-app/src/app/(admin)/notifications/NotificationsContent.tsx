    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="fashion-glass rounded-2xl p-6">
          <h3 className="text-base font-bold text-gray-900 mb-4">发送通知</h3>
          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">标题 *</label>
              <input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="例：系统维护公告"
                className="w-full px-3.5 py-2.5 rounded-2xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-200"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">内容</label>
              <textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder="通知详细内容（选填）"
                rows={3}
                className="w-full px-3.5 py-2.5 rounded-2xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-200 resize-none"
              />
            </div>

            {/* 发送对象 */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">发送对象</label>
                ...
                <div className="max-h-[200px] overflow-y-auto border border-gray-200 rounded-2xl p-2 flex flex-col gap-1">
                  {customers.filter((c) => c.role !== 'ADMIN').map((c) => (
                    <label key={c.id} className="flex items-center gap-2 px-2 py-1.5 rounded-2xl hover:bg-gray-50 cursor-pointer">
                      ...
                      <span className="text-xs text-gray-400 ml-auto">{c.credits} 积分</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 px-6 rounded-2xl text-white text-sm font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {/* 发送历史 */}
        <div className="fashion-glass rounded-2xl p-6">
          <h3 className="text-base font-bold text-gray-900 mb-4">发送历史</h3>
          {history.length === 0 ? (
              ...
              {history.map((n: any) => (
                <div key={n.id} className="p-3 bg-gray-50/60 rounded-2xl border border-gray-100">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${n.userId ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>
                      {n.userId ? n.targetEmail || '指定' : '全体'}
                    </span>
                  </div>
                  {n.content && <p className="text-xs text-gray-500 mb-1 line-clamp-2">{n.content}</p>}
