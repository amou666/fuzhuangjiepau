...
export async function PATCH(
  ...
  try {
    ...
    // 检查 apiKey 是否已被其他用户使用
    const existingUser = db.prepare('SELECT id FROM User WHERE apiKey = ? AND id != ?').get(apiKey.trim(), id) as any
    if (existingUser) {
      return NextResponse.json({ message: '该 API Key 已被其他用户使用，请换一个' }, { status: 409 })
    }

    db.prepare(`UPDATE User SET apiKey = ?, updatedAt = datetime('now') WHERE id = ?`).run(apiKey.trim(), id)
