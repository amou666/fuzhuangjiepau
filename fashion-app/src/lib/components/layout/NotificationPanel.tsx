  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        className="relative flex items-center justify-center w-9 h-9 rounded-2xl hover:bg-[var(--bg-active)] transition-colors"
        title="通知"
      >
        ...
        <div className="fixed md:absolute top-14 md:top-full right-2 md:right-auto left-2 md:left-full mt-0 md:mt-2 md:ml-2 md:w-[340px] max-h-[440px] bg-white/95 backdrop-blur-xl rounded-2xl border border-[var(--border-normal)] shadow-xl z-[200] overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-light)]">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                className="w-6 h-6 rounded-2xl flex items-center justify-center text-[var(--text-quaternary)] hover:bg-[var(--bg-active)] transition-colors"
          ...
          <div className="flex-1 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => {
                ...
                const color = TYPE_COLORS[n.type] || 'var(--text-secondary)'
                return (
                  <div
                    key={n.id}
                    className="px-4 py-3 border-b border-[rgba(139,115,85,0.04)] hover:bg-[var(--bg-muted)] transition-colors"
                    style={{ opacity: n.isRead ? 0.6 : 1 }}
                  >
                    <div className="flex gap-2.5">
                      <div
                        className="w-7 h-7 rounded-2xl flex items-center justify-center flex-shrink-0 mt-0.5"
