import type { GenerationTask, ModelConfig, SceneConfig } from '@/lib/types';
...
export function StepFusion({
  ...
  const getBodyLabel = (_body: string) => {
    const map: Record<string, string> = { slim: '修长', athletic: '匀称', curvy: '曲线' };
    return map[_body] || _body;
  };
  void getBodyLabel;

  const getCategoryLabel = (category: string) => {
    const map: Record<string, string> = { normal: '普通女孩', supermodel: '时尚超模', kardashian: '卡戴珊风格' };
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          ...
          >
            ...
            <span style={{ fontSize: '14px', color: clothingUrl ? '#059669' : '#ef4444', fontWeight: 500 }}>
              {clothingUrl
                ? `✓ 正面图${clothingBackUrl ? ' + 背面图' : ''}${clothingDetailUrls?.filter(Boolean).length ? ` + ${clothingDetailUrls.filter(Boolean).length}张细节图` : ''}`
                : '✗ 未上传'}
            </span>
          </div>
          ...
          >
            ...
            <span style={{ fontSize: '14px', color: '#374151' }}>
              {sceneConfig.mode === 'replace' && sceneConfig.imageUrl
                ? '替换模式（保留背景和姿势）'
                : sceneConfig.sceneSource === 'upload' && sceneConfig.imageUrl
                  ? '上传场景图'
                  : sceneConfig.preset}
            </span>
          </div>
