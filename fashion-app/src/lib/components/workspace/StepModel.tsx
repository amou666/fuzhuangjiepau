...
const poseOptions = [
  ...
  { value: 'power pose with legs apart（棚拍分腿力量站姿）' },
  { value: 'hand on waist chin up（棚拍叉腰仰头）' },
  { value: 'sitting on stool legs crossed（棚拍坐凳翘腿）' },
  { value: 'one arm raised behind head（棚拍单臂举过头顶）' },
  { value: 'dramatic lean on prop table（棚拍斜靠道具桌）' },
  { value: 'profile pose looking away（棚拍侧脸凝视远方）' },
  { value: 'high fashion hand near face（棚拍手部靠近面部）' },
  { value: 'contrapposto with hand in hair（棚拍对位站姿手撩发）' },
  { value: 'kneeling on one knee arm extended（棚拍单膝跪地手臂伸展）' },
  { value: 'back-to-camera over shoulder glance（棚拍背对镜头回眸）' },
];
    ...
    <>
      ...
      <div className="field" style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            type="button"
            className={`btn-secondary ${value.mode === 'generate' ? 'btn-active' : ''}`}
          ...
          <button
            type="button"
            className={`btn-secondary ${value.mode === 'upload' ? 'btn-active' : ''}`}
